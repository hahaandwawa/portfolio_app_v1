...
def _safe_quote_for_symbol(symbol: str, tickers_obj) -> tuple[Optional[float], str]:
    """
    Get current_price and display_name for one symbol from a yfinance Tickers object.
    Returns (current_price, display_name). On any error, returns (None, symbol).
    """
    try:
...
def _fetch_quotes_impl(symbols: list[str]) -> dict[str, dict]:
    """Call yfinance and return dict symbol -> { current_price, display_name }. No cache."""
    if not symbols:
        return {}
    yf = _get_yf()
    ...
    result = {}
    for sym in symbols:
        price, name = _safe_quote_for_symbol(sym, tickers)
        result[sym] = {"current_price": price, "display_name": name}
    return result
...
class QuoteService:
    ...
    def __init__(
        ...
        # Cache: symbol -> (current_price, display_name, cached_at)
        self._cache: dict[str, tuple[Optional[float], str, float]] = {}

    def get_quotes(self, symbols: list[str]) -> dict[str, dict]:
        """
        Return for each symbol: { "current_price": float | None, "display_name": str }.
        Uses cache when entry is within TTL; otherwise fetches (with timeout). On timeout
        or per-symbol failure, returns current_price=None and display_name=symbol.
        """
        if not symbols:
            return {}
        now = time.monotonic()
        ...
        for sym in symbols:
            ...
            if key in self._cache:
                price, name, cached_at = self._cache[key]
                if now - cached_at <= self._ttl:
                    result[key] = {"current_price": price, "display_name": name}
                    continue
            to_fetch.append(key)
        ...
        if to_fetch:
            try:
                ...
                fetched = {}
                for sym in to_fetch:
                    fetched[sym] = {"current_price": None, "display_name": sym}

            for sym in to_fetch:
                data = fetched.get(sym) or {"current_price": None, "display_name": sym}
                price = data.get("current_price")
                name = data.get("display_name") or sym
                result[sym] = {"current_price": price, "display_name": name}
                self._cache[sym] = (price, name, time.monotonic())
