...
class TestSafeQuoteForSymbol:
    """_safe_quote_for_symbol returns (price, name) or (None, symbol) on error."""
    ...
    def test_extracts_current_price_and_long_name(self):
        ...
        price, name = _safe_quote_for_symbol("AAPL", tickers)
        assert price == 175.5
    ...
    def test_fallback_regular_market_price(self):
        ...
        price, name = _safe_quote_for_symbol("X", tickers)
        assert price == 22.0
        assert name == "X Corp"
    ...
    def test_fallback_display_name_to_symbol(self):
        ...
        price, name = _safe_quote_for_symbol("Y", tickers)
        assert price == 10.0
        assert name == "Y"
    ...
    def test_missing_ticker_returns_none_and_symbol(self):
        tickers = MagicMock()
        tickers.tickers = {}
        price, name = _safe_quote_for_symbol("UNKNOWN", tickers)
    ...
    def test_exception_returns_none_and_symbol(self):
        ...
        price, name = _safe_quote_for_symbol("Z", tickers)
        assert price is None
        assert name == "Z"
...
def test_get_quotes_returns_price_and_name(mock_get_yf):
    """get_quotes calls yfinance and returns current_price and display_name per symbol."""
    mock_yf = MagicMock()
    mock_tickers = MagicMock()
    mock_tickers.tickers = {
        "AAPL": MagicMock(info={"currentPrice": 180.0, "longName": "Apple Inc."}),
...
def test_get_quotes_failure_returns_none_price_and_symbol_as_name(mock_get_yf):
    """One symbol raises when accessed; that symbol gets current_price=None, display_name=symbol."""
...
def test_get_quotes_exception_during_fetch_returns_none_and_symbol(mock_get_yf):
    """When fetch raises (e.g. timeout), get_quotes returns current_price=None, display_name=symbol."""
